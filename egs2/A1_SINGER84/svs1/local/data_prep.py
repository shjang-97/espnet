...
def read_midi_files_from_folder(folder_path):
    """
    폴더 안의 모든 MIDI 파일을 읽어 리스트에 저장합니다.

    Args:
        folder_path (str): MIDI 파일이 있는 폴더 경로.
    ...
    try:
        ...
        for filename in os.listdir(folder_path):
            if filename.endswith('.mid') or filename.endswith('.midi'):  # MIDI 확장자 확인
...
def load_midi(args):
    ...
    midi_path = os.path.join(args.src_data, "raw_data", "midis")
    midi_list = read_midi_files_from_folder(midi_path)
    
    midis = {}
    for midi_path in midi_list:
        i = midi_path.split('_')[-1].split('.')[0]
